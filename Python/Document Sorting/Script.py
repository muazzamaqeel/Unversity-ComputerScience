...
def organize_files():
    ...
    for file_name in os.listdir(current_directory):
        if os.path.isfile(file_name) and file_name != __file__:
    ...
    def display_results():
        ...
        if open_files:
            ...
            tk.Label(retry_window, text=open_files_message).pack(pady=10)
            tk.Button(retry_window, text="Try Again", command=retry_moving_files).pack(pady=5)
        else:
